...
import type { PackageJson, Split } from 'type-fest';
...
>({
	...
	const packageSlugpaths = {
		...
		[K in keyof typeof packageJsonPaths as K extends `${infer _PackageCategory}/${infer PackageSlug}/package.json`
			? CamelCase<Split<PackageSlug, '-'>>
			: never]: K extends `${infer PackageSlugpath}/package.json`
			? PackageSlugpath
			: never;
	} & { monorepo: '/monorepo' };
	...
	async function getPackageJson({ packageName }: { packageName: string }) {
		...
		const packageJson = JSON.parse(
			await fs.promises.readFile(
				path.join(getPackageDirpath({ packageName }), 'package.json'),
				'utf8'
			)
		);
